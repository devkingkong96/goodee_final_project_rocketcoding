...
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    const PHONE = window.PHONE = config => {
            ...
            let talk = conversations[number] || (number => {
                ...
                talk.pc.oniceconnectionstatechange = function () {
                    if (talk.pc.iceConnectionState == 'disconnected') {
                        let videoElements = document.querySelectorAll('video[playsinline]');
                        videoElements.forEach(element => {
                            let playsinlineValue = element.getAttribute('playsinline');
                            let playsinlineNumber = parseInt(playsinlineValue.replace('playsinline', ''));
                            if (playsinlineNumber !== number) {
                                // 해당 요소를 삭제
        ...
        function snapshots_setup(stream) {
            ...
            snapper = () => {
                try {
                    ...
                    debugcb(["context.drawImage(FAIL)", e]);
                }
        ...
        function onaddstream(number, obj) {
            ...
            vid.setAttribute('playsinline', 'playsinline' + number);
        ...
        function receive(message) {
            ...
            if (message.packet.hangup) {
                // 상대방의 비디오 요소를 찾아서 삭제
                let videoElement = document.querySelector(`video[playsinline='playsinline${message.number}']`);
